...
only authenticates, validates, enqueues, and returns 202. A background worker
runs the bounded agent loop, offloaded to a thread so the event loop stays free.
"""
...
def create_app(
    ...
    app.state.llm = llm
    app.state.tools = tools if tools is not None else {}
